module ApplicationHelper
  ...
  def get_random_documents(query: '*', limit: 12)
    ...
    docs = response[0][:response][:docs].collect { |x| x.slice(:id,:canonicalNameComplete_s,:commonname_s,:locality_s, :blob_ss)}
    return docs
  end
  ...
  def determine_flowering_month_color(flowering_month)
    if ['x','no'].include? flowering_month.to_s.downcase
      cell_class = 'table-secondary'
      text = 'not flowering'
    elsif flowering_month.downcase == 'some'
      cell_class = 'table-warning'
      text = 'some flowering'
    elsif flowering_month.downcase == 'many'
  ...
  def find_no_data(options)
    skip = false
    if options[:value].all? {|flowering| ['x','no'].include? flowering.to_s.downcase }
      skip = true
    end
    return skip
  end

  def render_flowering_calendar options = {}
    ...
    tbody = content_tag(:tbody) do
      content_tag(:tr) do
        options[:value].collect { |flowering_month|
          puts flowering_month
          flowering_status = determine_flowering_month_color(flowering_month)
          content_tag(:td, flowering_status[1],class: flowering_status[0])
        }.join().html_safe
      end
    end
